...
@section('content')
<!-- Page Heading -->
<h1 class="h3 mb-2 text-gray-800">Ubah Admin</h1>
<p class="mb-4">Halaman ini untuk mengubah admin pada aplikasi.</p>

<!-- DataTales Example -->
<div class="card shadow mb-4">
    <div class="card-body">
        <form class="user" action="{{ route('dashboard.admin.update',$admin) }}" method="POST">
            @method('PUT')
            @csrf
            <div class="form-group">
                <input type="text" class="form-control form-control-user @error('name') is-invalid @enderror" id="Name"
                    placeholder="Name" name="name" value="{{ old('name') ?? $admin->name }}">
                @error('name')
            ...
            <div class="form-group">
                <input type="email" class="form-control form-control-user @error('email') is-invalid @enderror"
                    id="Email" placeholder="Email Address" name="email" value="{{ old('email') ?? $admin->email }}">
                @error('email')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
